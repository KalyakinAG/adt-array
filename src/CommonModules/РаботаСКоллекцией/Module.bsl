...
Функция ТаблицаЗначений(Колонки, ПривестиСсылкуКСтроке = Ложь) Экспорт 
	Таблица = Новый ТаблицаЗначений;
	СоздатьКолонки(Таблица, Колонки, ПривестиСсылкуКСтроке);
	Возврат Таблица;
КонецФункции

Функция ДеревоЗначений(Колонки, ПривестиСсылкуКСтроке = Ложь) Экспорт 
	Дерево = Новый ДеревоЗначений;
	СоздатьКолонки(Дерево, Колонки, ПривестиСсылкуКСтроке);
	Возврат Дерево;
КонецФункции
...
Функция СтруктураВТаблицуЗначений(ТаблицаJSON, НайтиСсылкуИзСтроки = Истина) Экспорт
	ТипСтрока = Тип("Строка");
	Если ТипЗнч(ТаблицаJSON) = ТипСтрока Тогда
		Возврат СтруктураВТаблицуЗначений(ОбщийКлиентСервер.JSONВЗначение(ТаблицаJSON,, Ложь), НайтиСсылкуИзСтроки);
	КонецЕсли;
	Таблица = ТаблицаЗначений(ТаблицаJSON.Колонки, НЕ НайтиСсылкуИзСтроки);
...
Функция ДеревоЗначенийВСтруктуру(Дерево, ПривестиСсылкуКСтроке = Ложь) Экспорт
	СтруктураТаблицы = Новый Структура("Тип, Колонки, Строки", "ДеревоЗначений", Новый Массив, Новый Массив);
	Колонки = СтруктураТаблицы.Колонки;
	Для Каждого Колонка Из Дерево.Колонки Цикл
		Колонки.Добавить(Новый Структура("Имя, ТипЗначения", Колонка.Имя, Общий.СтроковоеПредставлениеТипа(Колонка.ТипЗначения)));
	КонецЦикла;
	ДобавитьСтрокиДерева(Колонки, СтруктураТаблицы.Строки, Дерево.Строки, ПривестиСсылкуКСтроке);
	Возврат СтруктураТаблицы;
КонецФункции

Функция СтруктураВДеревоЗначений(ДеревоJSON, НайтиСсылкуИзСтроки = Истина) Экспорт
	Если ТипЗнч(ДеревоJSON) = Тип("Строка") Тогда
		Возврат СтруктураВДеревоЗначений(ОбщийКлиентСервер.JSONВЗначение(ДеревоJSON,, Ложь), НайтиСсылкуИзСтроки);
	КонецЕсли;
	Дерево = ДеревоЗначений(ДеревоJSON.Колонки, НЕ НайтиСсылкуИзСтроки);
	Колонки = Дерево.Колонки;
	ЗагрузитьСтрокиДерева(Колонки, Дерево.Строки, ДеревоJSON.Строки, НайтиСсылкуИзСтроки);
	Возврат Дерево;
КонецФункции
...
Функция ДеревоВТаблицуЗначений(Дерево, КолонкаИД = "Идентификатор", КолонкаИДРодителя = "ИдентификаторРодителя") Экспорт
	Таблица = ТаблицаЗначений(Дерево.Колонки);
	Колонки = Таблица.Колонки;
	Если Колонки.Найти(КолонкаИД) = Неопределено Тогда
		Колонки.Добавить(КолонкаИД);
	КонецЕсли;
	Если Колонки.Найти(КолонкаИДРодителя) = Неопределено Тогда
		Колонки.Добавить(КолонкаИДРодителя);
	КонецЕсли;
	ДобавитьСтрокиДереваВТаблицуЗначений(Таблица, Дерево.Строки, КолонкаИД, КолонкаИДРодителя);
	Возврат Таблица;
КонецФункции
...
// ВJSON.
// Конвертирует коллекцию в структуру, сохраняет ссылки в виде идентификатора
// Параметры:
...
// 
// Возвращаемое значение:
//  Строка - ВJSON в формате КоллекцияЗначенийВСтруктуру()
Функция ВJSON(Коллекция, КрасивоеФорматирование = Ложь) Экспорт
	ТипЗначения = ТипЗнч(Коллекция);
	Если ТипЗначения = Тип("ТаблицаЗначений") Тогда
		Возврат ОбщийКлиентСервер.ЗначениеВJSON(ТаблицаЗначенийВСтруктуру(Коллекция, Истина), КрасивоеФорматирование);
	ИначеЕсли ТипЗначения = Тип("ДеревоЗначений") Тогда
		Возврат ОбщийКлиентСервер.ЗначениеВJSON(ДеревоЗначенийВСтруктуру(Коллекция, Истина), КрасивоеФорматирование);
	ИначеЕсли ТипЗначения = Тип("Структура") И Коллекция.Свойство("Тип") Тогда
		Возврат ОбщийКлиентСервер.ЗначениеВJSON(Коллекция, КрасивоеФорматирование);
	Иначе
...
// Из JSON.
// Восстанавливает коллекцию, идентификаторы заменяет ссылками
// 
// Параметры:
//  КоллекцияJSON - Строка - Коллекция JSON в формате КоллекцияЗначенийВСтруктуру()
// 
// Возвращаемое значение:
//  ДеревоЗначений, ТаблицаЗначений - Из JSON
Функция ИзJSON(КоллекцияJSON) Экспорт
	Перем ТипЗначения;
	Коллекция = ОбщийКлиентСервер.JSONВЗначение(КоллекцияJSON,, Ложь);
	Коллекция.Свойство("Тип", ТипЗначения);
	Если НЕ ЗначениеЗаполнено(ТипЗначения) ИЛИ ТипЗначения = "ТаблицаЗначений" Тогда
		Возврат СтруктураВТаблицуЗначений(Коллекция, Истина);
	ИначеЕсли ТипЗначения = "ДеревоЗначений" Тогда
		Возврат СтруктураВДеревоЗначений(Коллекция, Истина);
	Иначе
		ВызватьИсключение "Неизвестный тип коллекции " + ТипЗначения;
	КонецЕсли;
КонецФункции
...
#Область РаботаСИдентификатором
...
Процедура СоздатьКолонки(Коллекция, Колонки, ПривестиСсылкуКСтроке = Ложь)
	...
	ИначеЕсли ТипЗначения = Тип("Массив") Тогда
		...
		Если ТипЗнч(Колонка0) = Тип("Структура") Тогда
			
			Если ТипЗнч(Колонка0.ТипЗначения) = Тип("Строка") Тогда
				
				Для каждого Колонка Из Колонки Цикл
					Коллекция.Колонки.Добавить(Колонка.Имя, Общий.ТипЗначенияИзСтроки(Колонка.ТипЗначения, ПривестиСсылкуКСтроке));
				КонецЦикла;
				
			Иначе
				
				Для каждого Колонка Из Колонки Цикл
					Коллекция.Колонки.Добавить(Колонка.Имя, Колонка.ТипЗначения);
				КонецЦикла;
...
Процедура ДобавитьСтрокиДерева(Колонки, Строки, СтрокиДерева, ПривестиСсылкуКСтроке)
	Для Каждого Строка Из СтрокиДерева Цикл
		...
		ДобавитьСтрокиДерева(Колонки, ПодчиненныеСтроки, Строка.Строки, ПривестиСсылкуКСтроке);
...
Процедура ДобавитьСтрокиДереваВТаблицуЗначений(Таблица, Строки, КолонкаИД, КолонкаИДРодителя, ИДРодителя = -1)
	Для Каждого Строка Из Строки Цикл
		НоваяСтрока = Таблица.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		НоваяСтрока[КолонкаИДРодителя] = ИДРодителя;
		НоваяСтрока[КолонкаИД] = Таблица.Индекс(НоваяСтрока);
		ДобавитьСтрокиДереваВТаблицуЗначений(Таблица, Строка.Строки, КолонкаИД, КолонкаИДРодителя, НоваяСтрока[КолонкаИД]);
	КонецЦикла;
КонецПроцедуры
