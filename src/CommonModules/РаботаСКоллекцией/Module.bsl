...
Функция CSVвСтруктуру(Текст, Знач Разделитель = "Таб") Экспорт
	Если ВРег(Разделитель) = "ТАБ" Тогда
		Разделитель = Символы.Таб;
	КонецЕсли;
	СтруктураТаблицы = Новый Структура("Тип, Колонки, Строки", "ТаблицаЗначений", Новый Структура, Новый Массив);
	Строки = СтрРазделить(Текст, Символы.ПС);
	Шапка = РаботаСМассивом.Сдвинуть(Строки);
	СтруктураТаблицы.Колонки = РаботаСМассивом.Преобразовать(СтрРазделить(Шапка, Разделитель), "ОбщийКлиентСервер.ВставитьСвойство(Накопитель, Элемент, Неопределено)", Новый Структура);
...
Функция ВMarkdown(Коллекция) Экспорт
	...
	КонецЕсли;
	Колонки = РаботаСМассивом.Отобразить(Коллекция.Колонки, "Элемент.Ключ");
	Строки = Новый Массив;
