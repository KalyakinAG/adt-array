...
// 
// Параметры:
//  Список - Массив из Произвольный - любая коллекция, которая поддерживает Для Каждого 
...
Функция НайтиЭлемент(Список, ВыражениеПредиката, Контекст = Неопределено, Параметры = Неопределено) Экспорт
...
Функция Индекс(Список, ВыражениеПредиката, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	Выражение = Нормализовать(ВыражениеПредиката);
...
Функция Отобрать(Список, ВыражениеПредиката, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	Выражение = Нормализовать(ВыражениеПредиката);
...
Функция Отобразить(Список, ВыражениеОтображения, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	...
	Если ТипЗнч(Список) = Тип("Массив") Тогда
		Для Индекс = 0 По Список.ВГраница() Цикл
			Элемент = Список[Индекс];
...
Функция Преобразовать(Список, ВыражениеПреобразования, НачальноеЗначение = Неопределено, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
...
//  Параметры - Произвольный
// 
Процедура ДляКаждого(Список, АлгоритмОбработки, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
...
Функция Первый(Список) Экспорт
	ВГраница = Список.ВГраница();
...
// 
// Параметры:
//  Список - Массив из Произвольный
// 
// Возвращаемое значение:
//  Произвольный
Функция Последний(Список) Экспорт
	ВГраница = Список.ВГраница();
	Если ВГраница = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Результат = Список[ВГраница];
	Возврат Результат;
КонецФункции
...
// Возвращаемое значение:
//  Произвольный
Функция Взять(Список) Экспорт
	ВГраница = Список.ВГраница();
	Если ВГраница = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Результат = Список[ВГраница];
	Список.Удалить(ВГраница);
	Возврат Результат;
КонецФункции
...
// Возвращаемое значение:
//  Произвольный
Функция Сдвинуть(Список) Экспорт
	ВГраница = Список.ВГраница();
	Если ВГраница = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция Добавить(Список, Элемент) Экспорт
...
// Возвращаемое значение:
//  Массив из Произвольный
Функция Срез(Список, Знач НижняяГраница = 0, Знач ВерхняяГраница = Неопределено, Шаг = 1) Экспорт
	Результат = Новый Массив;
	Если НижняяГраница < 0 Тогда
		НижняяГраница = Список.Количество() + НижняяГраница;
		Если НижняяГраница < 0 Тогда
			НижняяГраница = 0;
		КонецЕсли;
	КонецЕсли;
	Если ВерхняяГраница = Неопределено ИЛИ ВерхняяГраница > Список.Количество() Тогда
...
// Возвращаемое значение:
//  Булево - Следующий
Функция Следующий(Индекс, Элемент, Список, Шаг = 1) Экспорт
	Если Индекс = Неопределено Тогда
		Индекс = 0;
	Иначе
		Индекс = Индекс + Шаг;
	КонецЕсли;
	Если Индекс > Список.ВГраница() Тогда
		Возврат Ложь;
	КонецЕсли;
	Элемент = Список[Индекс];
	Возврат Истина;
...
Функция Спрямить(Список, Глубина = Неопределено) Экспорт
...
// 
// Параметры:
//  Список - Массив из Структура - любая коллекция, которая поддерживает Для Каждого
...
Функция Свернуть(Список, ПоляГруппировки = Неопределено, ПоляСвертки = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПоляГруппировки) Тогда
		...
		Для Каждого Элемент Из Список Цикл
			...
			Для Каждого ИмяПоляСвертки Из СписокПолейСвертки Цикл
				Если ИмяПоляСвертки = "*" Тогда
					Группировка["Элемент"].Добавить(Элемент);
				Иначе
	...
	Результат = Дополнить(Новый Массив, Список, Истина);
	Возврат Результат;
...
Функция Максимум(Список) Экспорт
	Результат = 0;
	Для Каждого Элемент Из Список Цикл
...
Функция Минимум(Список) Экспорт
	Результат = 0;
	Для Каждого Элемент Из Список Цикл
		Если Элемент < Результат Тогда
			Результат = Элемент;
...
Функция Среднее(Список) Экспорт
	Возврат Сумма(Список) / Список.Количество();
КонецФункции

Функция Количество(Список) Экспорт
	Возврат Список.Количество();
КонецФункции

Функция КоличествоРазличных(Список, ПоляГруппировки = Неопределено) Экспорт
	Возврат Свернуть(Список, ПоляГруппировки).Количество();
КонецФункции
...
Функция АТДМассив(Список = Неопределено) Экспорт
	#Если Клиент Тогда
...
Функция ВСтруктуру(Список, ПолеКлюча = "Ключ", ПолеЗначения = "Значение") Экспорт
...
Функция ВСоответствие(Список, ПолеКлюча = "Ключ", ПолеЗначения = "Значение") Экспорт
...
Функция ОтобразитьСвойства(Список, СоответствиеСвойств, ПутьОтображения = Неопределено) Экспорт
	Результат = Новый Массив;
	Для Каждого Элемент Из Список Цикл
...
Функция ВставитьСвойство(Список, Свойство, ВыражениеОтображения, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	Выражение = Нормализовать(ВыражениеОтображения);
...
Функция ВставитьСвойствоРекурсивно(Список, Путь, ВыражениеОтображения) Экспорт
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	Выражение = Нормализовать(ВыражениеОтображения);
...
// рекурсивном вызове типа СортироватьПо(Отобразить(Список, 'Элемент.Значение'), "Имя")
// 
// Параметры:
//  Список - Массив из Произвольный - 
//  Поля - Строка - поля через запятую с указанием направления сортировки +/-
...
Функция СортироватьПо(Список, Знач Поля) Экспорт
	ПоляСравнения = ПоляСравнения(Поля);
	БыстраяСортировкаПо(Список, ПоляСравнения, 0, Список.ВГраница());
	Возврат Список;
КонецФункции

// sort
// Процедура меняет исходный массив и возвращает его же. Возврат может быть использован в
// рекурсивном вызове типа Сортировать(Отобразить(Список, 'Элемент.Ключ'))
// 
// Параметры:
//  Список - Массив из Произвольный - 
...
Функция Сортировать(Список, Знач ФункцияСравнения = Неопределено, Контекст = Неопределено, Параметры = Неопределено) Экспорт
	Если НЕ ЗначениеЗаполнено(ФункцияСравнения) ИЛИ ФункцияСравнения = "+" Тогда
		СписокЗначений = Новый СписокЗначений;
		СписокЗначений.ЗагрузитьЗначения(Список);
		СписокЗначений.СортироватьПоЗначению();
		Список = СписокЗначений.ВыгрузитьЗначения();
		Возврат Список;
	...
	БыстраяСортировка(Список, ФункцияСравнения, Контекст, Параметры, 0, Список.ВГраница());
	Возврат Список;
КонецФункции

// Сортировать Список дерева данных формы
// 
// Параметры:
//  Список - ДанныеФормыКоллекцияЭлементовДерева - 
//  ФункцияСравнения - Строка - см. Сравнить
//  СортироватьВложенные - Булево
Процедура СортироватьЭлементыКоллекции(Список, ФункцияСравнения = Неопределено, СортироватьВложенные = Истина) Экспорт
	...
	Если СортироватьВложенные Тогда
		Для Каждого ЭлементДерева Из Список Цикл
			СортироватьЭлементыКоллекции(ЭлементДерева.ПолучитьЭлементы(), Выражение, СортироватьВложенные);
		КонецЦикла;
	КонецЕсли;
	СортироватьЭлементы(Список, Выражение, 0, Список.Количество() - 1);
КонецПроцедуры
...
//  Словарь - Соответствие из Произвольный - используется для поиска элемента по ключу, если подчиненные Список содержат только ключи
...
Функция ОпределитьТопографическийПорядок(Список, Ключ = "Идентификатор", СвойствоПодчиненныхЭлементов = "ПодчиненныеЭлементы", СвойствоПорядка = "Порядок", Словарь = Неопределено, СвойствоЗначенияСловара = Неопределено) Экспорт
	ТипКоллекции = ТипЗнч(Список);
	...
	Если НЕ ЗначениеЗаполнено(Словарь) Тогда
		...
		Для Каждого ЭлементИлиКлючЗначение Из Список Цикл
	...
	Иначе
		...
		КонецЦикла;
	КонецЕсли;
	Возврат ОпределитьПорядок(Список, СвойствоПорядка, СвойствоПодчиненныхЭлементов, РаботаСОчередью.ОчередьУникальныхЗначений(Ключ), Словарь, СвойствоЗначенияСловара)
КонецФункции
...
// все Список первого массива, не существующие во втором массиве.
//
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция Содержит(Список, Значение) Экспорт
...
// 
// Параметры:
//  Очередь - Массив из Произвольный - Список очереди, по-умолчанию отсортированы от меньшего к большему
//  Элемент - Произвольный - 
//  ФункцияСравнения - Строка - см. Сравнить
//  Контекст - Произвольный
//  Параметры - Произвольный
...
// Возвращаемое значение:
//  Массив из Произвольный - новый массив
Функция СкопироватьМассив(Список) Экспорт
	Результат = Новый Массив;
	Для Каждого Элемент Из Список Цикл
		Результат.Добавить(Элемент);
	КонецЦикла;
...
// Возвращаемое значение:
//  Массив из Произвольный
Функция Положить(Список, Элемент) Экспорт
	Список.Добавить(Элемент);
	Возврат Список;
КонецФункции
...
Функция НовыйГруппировка(ПоляГруппировки = Неопределено, ПоляСвертки)
	...
	Для Каждого ИмяПоля Из Массив(ПоляСвертки) Цикл
		Если ИмяПоля = "*" Тогда
			Результат.Вставить("Элемент", Новый Массив);
			Продолжить;
		КонецЕсли;
...
Функция ОпределитьПорядок(Список, СвойствоПорядка, СвойствоПодчиненныхЭлементов, Очередь, Словарь, СвойствоЗначенияСловара, Знач Порядок = 0)
	ТипКоллекции = ТипЗнч(Список);
	ЭтоКлючЗначение = (ТипКоллекции = Тип("Соответствие") ИЛИ ТипКоллекции = Тип("Структура"));
	Для Каждого ЭлементИлиКлючЗначение Из Список Цикл
		//  Нужно проверить тип элемента. Если это подчиненные Список, то это ключ, верхнего - это сами Список
		Если ЭтоКлючЗначение Тогда
			Элемент = ЭлементИлиКлючЗначение.Значение;
		ИначеЕсли ТипЗнч(ЭлементИлиКлючЗначение) = Тип("Строка") Тогда
			Если ЗначениеЗаполнено(СвойствоЗначенияСловара) Тогда
		...
		Если РаботаСОчередью.Содержит(Очередь, Элемент) Тогда
			ЭлементыСтека = Отобразить(Очередь.Список, СтрШаблон("Элемент['%1']", Очередь.Ключ));
...
Процедура БыстраяСортировка(Список, ФункцияСравнения, Контекст, Параметры, Нижний, Верхний)
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
	Если Нижний < Верхний Тогда
		...
		Значение = Список[ОпорныйИндекс];
		Список[ОпорныйИндекс] = Список[Верхний];
		Список[Верхний] = Значение;
		Б = Список[Верхний];// опорное значение
		...
		Для Индекс = Нижний По Верхний Цикл
			...
			Если Вычислить(ФункцияСравнения) >= 0 Тогда // значение меньше или равно опорному в Б (0, 1)
				...
				Значение = Список[ИндексРаздела];
				Список[ИндексРаздела] = Список[Индекс];
				Список[Индекс] = Значение;
			КонецЕсли;
		КонецЦикла;
		БыстраяСортировка(Список, ФункцияСравнения, Контекст, Параметры, Нижний, ИндексРаздела - 1);
		БыстраяСортировка(Список, ФункцияСравнения, Контекст, Параметры, ИндексРаздела + 1, Верхний);
	КонецЕсли;
КонецПроцедуры

// Быстрая сортировка по полям элементов списка
// 
// Параметры:
//  Список - Массив из Произвольный - Список
//  ПоляСравнения - см. ПоляСравнения
//  Нижний - Число - Нижний
//  Верхний - Число - Верхний
Процедура БыстраяСортировкаПо(Список, ПоляСравнения, Нижний, Верхний)
	Перем Параметры;
	Стек = Новый Массив;
	Стек.Добавить(Новый Структура("Нижний, Верхний", Нижний, Верхний));
	Пока Стек.Количество() > 0 Цикл
		Параметры = Стек[Стек.Количество() - 1];
		Стек.Удалить(Стек.Количество() - 1);
		Нижний = Параметры.Нижний;
		Верхний = Параметры.Верхний;
		Если Нижний < Верхний Тогда
			ОпорныйИндекс = Цел((Верхний + Нижний) / 2);
			// перенос опорного значения в верхний индекс
			Значение = Список[ОпорныйИндекс];
			...
			Для Индекс = Нижний По Верхний Цикл
				...
				КонецЕсли;
			КонецЦикла;
			Стек.Добавить(Новый Структура("Нижний, Верхний", Нижний, ИндексРаздела - 1));
			Стек.Добавить(Новый Структура("Нижний, Верхний", ИндексРаздела + 1, Верхний));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СортироватьЭлементы(Список, ВыражениеПредиката, НижнийПредел, ВерхнийПредел)
	#Если НЕ ТонкийКлиент Тогда
		УстановитьБезопасныйРежим(Истина);
	#КонецЕсли
    Начало = НижнийПредел;
    Конец = ВерхнийПредел;
    Б = Список[Цел((Начало + Конец)/2)];
	Пока Истина Цикл
		А = Список[Начало];
        Пока Вычислить(ВыражениеПредиката) < 0 Цикл // Список[Начало] < Б
            Начало = Начало + 1;                   
			А = Список[Начало];
		КонецЦикла;
		А = Список[Конец];
        Пока Вычислить(ВыражениеПредиката) > 0 Цикл // Список[Конец] > Б
            Конец = Конец - 1;                   
			А = Список[Конец];
        КонецЦикла; 
        Если Начало <= Конец Тогда               
			...
			Если НЕ ШагСдвига = 0 Тогда
				Список.Сдвинуть(Начало, ШагСдвига);
				ШагСдвига = ШагСдвига - 1;
				Если НЕ ШагСдвига = 0 Тогда
					Список.Сдвинуть(Конец - 1, -ШагСдвига);
				КонецЕсли;
			КонецЕсли;
            //  Дерево значений            
            Начало = Начало + 1;
    ...
    Если НижнийПредел < Конец Тогда         
        СортироватьЭлементы(Список, ВыражениеПредиката, НижнийПредел, Конец);        
	КонецЕсли; 
    Если Начало < ВерхнийПредел Тогда                      
        СортироватьЭлементы(Список, ВыражениеПредиката, Начало, ВерхнийПредел);        
    КонецЕсли;
КонецПроцедуры
